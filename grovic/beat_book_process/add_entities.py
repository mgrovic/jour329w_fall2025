...
- impact: which topic is most impacted by this story
...
Example output:
{{
  "people": [{"name":"Donald Trump", "title": "President of the United States"}, {"name":"Ben Cardin", "title": "Former Maryland Senator"}, {"name":"Andrew Barnet", "title": "Co-Owner of Open Book Farm"}],
  ...
  "impact": ["Economy" , "Culture", "Policy", "Education", etc]
}}
...
def coerce_entities(obj: Dict[str, Any]) -> Dict[str, Any]:
    # Map accidental keys from common alternatives
    ...
    return {
        ...
        "people": clean_list(obj.get("people", []))[:6],  # cap at 6 people
...
def main():
    ap = argparse.ArgumentParser(description="Extract entities (people, geographic_focus, key_institutions, environmental_focus) via llm CLI.")
